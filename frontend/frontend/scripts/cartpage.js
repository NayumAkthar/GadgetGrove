document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchCart() {
    ...
    try {
      const res = await fetch(`https://gadgetgrove-n38v.onrender.com/api/cart/${userId}`);
  ...
  confirmRemoveBtn.addEventListener('click', async () => {
    if (itemToRemove) {
      try {
        await fetch(`https://gadgetgrove-n38v.onrender.com/api/cart/${itemToRemove}`, { method: 'DELETE' });
  ...
  clearCartBtn.addEventListener('click', async () => {
    ...
    for (let item of cartItems) {
      await fetch(`https://gadgetgrove-n38v.onrender.com/api/cart/${item.id}`, { method: 'DELETE' });
    }
    fetchCart();
  });
