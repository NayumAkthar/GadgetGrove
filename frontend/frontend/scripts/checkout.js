document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const response = await fetch(`https://gadgetgrove-n38v.onrender.com/api/product/${encodeURIComponent(productName)}`);
...
async function saveAddress() {
  ...
  try {
    const response = await fetch('https://gadgetgrove-n38v.onrender.com/api/address', {
      method: 'POST',
...
async function fetchAddress(userId) {
  try {
    const response = await fetch(`https://gadgetgrove-n38v.onrender.com/api/address/${userId}`);
    const data = await response.json();
